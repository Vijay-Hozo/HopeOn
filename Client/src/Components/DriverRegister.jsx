...
const UserRegister = () => {
    ...
    const handleregister = async(e) => {
      e.preventDefault();
      try{
      const res = await axios.post("http://localhost:3000/registerdriver",{
          driver_name : name,
          driver_email : email,
        ...
        <div className='flex bg-background-primary bg-gradient-to-b from-background-primary to-background-secondary'>
            ...
            <form className='flex flex-col items-center text-xl gap-4 max-h-full w-full text-blue-950 font-semibold py-5' onSubmit={handleregister}>
                ...
                <div>
                    ...
                    <button type="button" className="text-white bg-[#043e9a] hover:bg-[#4285F4]/90 focus:ring-4 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55 mr-2 mb-2">
                ...
                <div>
                    ...
                    <span className='text-yellow-400'><a href="/driverlogin"> Login</a></span>
                </div>
            </form>
        </div>
    );
