...
const RideCard = ({ ride }) => {
  ...
  const handleRequest = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:3000/request", payload, {
        headers: {
    ...
    <div className='flex flex-col items-center bg-slate-200 w-[500px] p-8 h-auto m-8 rounded-xl gap-4 text-black'>
      ...
      <div className='flex justify-center'>
        {requestStatus === "requested" ? (
          <h1 className='bg-blue-600 text-white font-semibold p-3 rounded-2xl'>Waiting for Confirmation</h1>
        ) : (
